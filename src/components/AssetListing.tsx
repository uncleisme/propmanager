...
interface Asset {
  ...
  asset_name: string;
  asset_type: string | null;  // Reference to asset_types table
...
const AssetListing: React.FC<AssetListingProps> = () => {
  ...
  const fetchAssets = useCallback(async () => {
    ...
    try {
      ...
      const { data: assetsData, error: assetsError } = await supabase
        .from("assets")
        .select('*')
        .or(`asset_name.ilike.%${search}%`);

      if (assetsError) throw assetsError;
      
      // Get all asset types for mapping
      const { data: assetTypesData, error: typesError } = await supabase
        .from("asset_types")
        .select('id, name, category');
        
      if (typesError) throw typesError;
      
      // Create a map of asset types for quick lookup
      const assetTypesMap = new Map();
      (assetTypesData || []).forEach(type => {
        assetTypesMap.set(type.id, type);
      });
      
      // Transform the data to match our Asset interface
      const transformedAssets = (assetsData || []).map(asset => {
        const assetType = assetTypesMap.get(asset.asset_type);
        return {
          ...asset,
          asset_type_name: assetType?.name || null,
          asset_type_category: assetType?.category || null
        };
      });
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    if (name === 'parent_asset_id' || name === 'asset_type') {
      setFormData(prev => ({
        ...prev,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Refresh the assets list to get properly transformed data
      await fetchAssets();
      setShowModal(false);
      setErrorMsg("");
    } catch (error: any) {
